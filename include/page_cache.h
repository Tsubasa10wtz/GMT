...
// CHIA-HAO
#define USE_HOST_CACHE 1 // USE DRAM
#define ENABLE_TMM 1
#define PRINT_TMM 0
...
//        printf("Cache Extra Reads: %llu\n", v);
    ...
    page_cache_t(const uint64_t ps, const uint64_t np, const uint32_t cudaDevice, const uint64_t max_range) {
        ...
        // ranges_buf = createBuffer(max_range, cudaDevice);
        ...
        cudaError_t err = cudaMalloc(&device_cache, cache_size);
        if (err != cudaSuccess) {
            throw std::runtime_error("Failed to allocate device memory: " + std::string(cudaGetErrorString(err)));
        }
        pdt.base_addr = reinterpret_cast<uint8_t*>(device_cache);

        printf("[ALLOC] device_cache base: %p, size = %lu, end = %p\n",
        device_cache, cache_size,
        (void*)((uint8_t*)device_cache + cache_size));
...
uint32_t cudaDevice, data_dist_t dist) {
    ...
    for (size_t i = 0; i < s; i++) {
        // ts[i].state = INVALID | CACHED_BY_HOST;
        ts[i].state = INVALID;
    }
...
uint64_t range_d_t<T>::find_page(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl_, const uint32_t queue, bool& nv_nb_and_cached_by_hc) {
    uint64_t index = pg;
    // printf("[DEBUG] find_page(): page = %lu\n", ((range_id<<32)|index));
    access_cnt.fetch_add(count, simt::memory_order_relaxed);
    bool fail = true;
    ...
    // if ((read_state & CACHED_BY_HOST) == 0x0) {
    //     printf("Page not in DRAM. Page: %lu\n", ((range_id<<32)|index));
    // }
        ...
        case NV_NB:
        {
            ...
            if ((st_new & BUSY) == 0) {
                ...
                if ((st_new & CACHED_BY_HOST) == 0x0) {
                    ...
                    // 打印修改前的 VALID 位状态
                    uint32_t state_before = pages[index].state.load(simt::memory_order_relaxed);
                    // printf("[Before fetch_xor] Page %lu: VALID = %u, state = 0x%x\n", ((range_id << 32) | index), (state_before & VALID) != 0, state_before);

                    // 做 VALID = 1 & 清 BUSY 的转换
                    pages[index].state.fetch_xor(DISABLE_BUSY_ENABLE_VALID, simt::memory_order_release);

                    // 打印修改后的 VALID 位状态
                    uint32_t state_after = pages[index].state.load(simt::memory_order_acquire);
                    // printf("[After fetch_xor] Page %lu: VALID = %u, state = 0x%x\n", ((range_id << 32) | index), (state_after & VALID) != 0, state_after);
                    // pages[index].state.fetch_xor(DISABLE_BUSY_ENABLE_VALID, simt::memory_order_release);
...
uint32_t page_cache_d_t::find_slot(uint64_t address, uint64_t range_id, const uint32_t /*queue_*/, int32_t* bid) {
    ...
    uint64_t global_address = ((uint64_t)range_id << 32) | address;

    // printf("[DEBUG] global_address in find_slot = %lu\n", global_address);
    uint32_t page = 0;
    unsigned int ns = 8;
    // printf("n_pages in find_slot: %d\n", this->n_pages);

    do {
        // tzwang
        // if (count == 0) {  // 只在第一次进循环时统计
        //     int used_slots = 0;
        //     for (uint32_t i = 0; i < this->n_pages; ++i) {
        //         uint32_t lock_val = this->cache_pages[i].page_take_lock.load(simt::memory_order_relaxed);
        //         if (lock_val != FREE) used_slots++;
        //     }
        //     printf("[find_slot] Used cache slots: %d\n", used_slots);
        //     printf("[find_slot] Total cache slots (n_pages): %d\n", this->n_pages);
        // }


        page = page_ticket->fetch_add(1, simt::memory_order_relaxed) % this->n_pages;
        // printf("[DEBUG] page_ticket = %u\n", page);
        ...
        if (v == FREE || v == UNLOCKED) {
            // printf("[DEBUG] enter v = FREE || v == UNLOCKED\n");
            ...
            if (lock) {
                if (v == UNLOCKED) {
                    ...
                    uint64_t prev_gaddr = this->cache_pages[page].page_translation;
                    // uint32_t prev_addr = prev_gaddr & n_ranges_mask;
                    uint32_t prev_addr = prev_gaddr & 0xFFFFFFFF;
                    // uint32_t prev_range  = prev_gaddr >> n_ranges_bits;
                    uint32_t prev_range  = prev_gaddr >> 32;
                    // printf("[DEBUG] Cache page to evict: prev_gaddr = %lu\n", prev_gaddr);
                    // printf("[DEBUG] Cache page to evict 2: prev_gaddr = %lu\n", (((uint64_t)prev_range << 32) | prev_addr));

                    uint32_t state = this->ranges[prev_range][prev_addr].state.load(simt::memory_order_relaxed);


                    if ((state & CNT_MASK) == 0 && (state & BUSY) == 0) {
                        // printf("[DEBUG] (state & CNT_MASK) == 0 && (state & BUSY) == 0");
                        ...
                        if (((old_state & BUSY) == 0) && ((old_state & CNT_MASK) == 0)) {
                            ...
                            if (tier == Tier2_CPU && (!idle_slots_too_few(0))) {
                                ...
                                if (ret == HOST_CACHE_SUCCEEDED) {
                                    ...
                                    this->ranges[prev_range][prev_addr].state.fetch_and(~(VALID), simt::memory_order_acquire);
                                }
                            }
                            #endif
                            ...
                            this->ranges[prev_range][prev_addr].state.fetch_and(DISABLE_BUSY_MASK, simt::memory_order_release);
                        } else {
